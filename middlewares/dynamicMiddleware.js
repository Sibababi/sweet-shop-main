...
exports.setPathImginBody =
 ...
 return next();
  ...
  exports.setPathImginBodys = (folder, ...filedImg) => (req, res, next) => {
      // تحقق إذا كانت الملفات موجودة
      if (!req.files || req.files.length === 0) {
        return next();
      }
    
      // قم بتخزين المسارات في req.body
      filedImg.forEach((item) => {
        if (req.files[item] && req.files[item].length > 0) {
          req.body[item] = `${req.protocol}://${req.get("host")}/img/${folder}/${req.files[item][0].filename}`;
        }
      });
