...
exports.getTotalUsers = async (req, res) => {
    try {
        const count = await  User.countDocuments({ role: 'user' });// لحساب عدد الوثائق في مجموعة المستخدمين
...
exports.getTotalDeleverys = async (req, res) => {
    try {
        const count = await  User.countDocuments({ role: 'delivery' });// لحساب عدد الوثائق في مجموعة المستخدمين
        res.status(200).json({ totalUsers: count });
    } catch (error) {
        console.error(error);
        res.status(500).json({ message: 'حدث خطأ أثناء حساب عدد المستخدمين' });
